  ...
  <body>
        				...
        				'<div class="login w3">'.
                			'<div id="horizontalTab" style="display: block; width: 100%; margin: 0px;">'.
								'<div class="resp-tabs-container">'.
                    				'<h2 align="center">'.$_SESSION["name"]."的个人中心".'</h2><br/>'.
									'<p style="text-align:center">'.
									'<a href="logout.php" class="btn btn-primary buttonstyle">注销</a>'.'<a href="add.php" class="btn btn-primary buttonstyle">发布内容</a>'.
									'</p>'.
								'</div>'.
                			'</div>'.
        				'</div>'.
	...
	<div class="container-fluid">
		<div class="row content div2">
			<div class="divc col-sm-12">
				<h3 class="btn btn-success buttonstyle">已发布内容</h3><br/>
				<?php
				/** 获取页面 **/
				...
				$sql = "SELECT * FROM posts where post_author='$author' order by post_id desc LIMIT ".($page-1)*$PageSize .",$PageSize";
				if(!$sql){
 				echo "取出不成功";
				};
				//把sql语句传送到数据库
				...
				while($row = mysqli_fetch_assoc($result)){
					echo 
						...
						'<td>'.'<a class="btn btn-primary" href="upload/'.$row['file'].'">下载</a>'.'</td>'.
						'<td>'.'<a class="btn btn-danger" href="del.php?id='.$row['post_id'].'" onclick="javascript:return p_del()">删 除</a></td>'.
						'</tr>'.
						'</table>'.
						'</div>';
					}
					...
					//获取数据总数
					$to_sql="SELECT COUNT(*)FROM posts where post_author='$author'";
