			...
			<div class="container-fluid">
				<div class="row content div2">
					<div class="divc col-sm-12">
						<h3 class="btn btn-success buttonstyle">全部内容</h3>
						<!--数据库调用代码-->
						<?php
							$search=$_GET['search'];
							/** 获取页面 **/
							...
							$sql = "SELECT * FROM posts WHERE post_title LIKE '%$search%' order by post_id desc LIMIT ".($page-1)*$PageSize .",$PageSize";
							if(!$sql){
							echo "取出不成功";
							};
							//把sql语句传送到数据库
							...
							while($row = mysqli_fetch_assoc($result)){
								echo 
									...
									'<a class="btn btn-primary" href="upload/'.$myrow['file'].'">下载</a>'.
									'</td>'.
									'</tr>'.
									'</table>'.
									'</div>';
								...
								//获取数据总数
								$to_sql="SELECT COUNT(*) FROM posts where post_title LIKE '%$search%'";
